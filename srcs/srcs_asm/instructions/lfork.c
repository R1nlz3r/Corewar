...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/09 15:59:39 by kda-silv          #+#    #+#             */
/*   Updated: 2018/03/13 17:24:14 by kda-silv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++count < 3)
		if (count == 0)
		{
			if (data->args[count] != DIR)
				asm_error("Lfork: Not DIRECT Argument", 2, data, line);
		}
		else
		{
			if (data->args[count] != -1)
				asm_error("Lfork: Only 1 Argument", 2, data, line);
		}
}
...
void			lfork(t_data *data, char *line, char **tab, int count_word)
{
	if (tab[count_word] == NULL)
		asm_error("Lfork: Argument missed", 2, data, line);
	if (tab[count_word][0] == 'r' || tab[count_word][0] == DIRECT_CHAR
		...
		asm_error("Lfork: Bad Arguments", 2, data, line);
	good_args(data, line);
}

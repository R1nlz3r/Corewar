...
/*   Updated: 2018/04/20 19:31:35 by cfrouin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	verbose_lfork(t_data *data, t_champion *champ,
								t_champion *child)
{
	char	color[7];
...
with no valid argument type\n", champ->number);
	...
	pc_dest = mem_mod(champ->oldpc->id + mem_address(champ->args[0]));
	...
	verbose_lfork(data, champ, child);
	return (1);
}
